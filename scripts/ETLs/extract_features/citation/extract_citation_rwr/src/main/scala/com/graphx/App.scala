...
object App {
    ...
    val dst_dir                 = "gs://clpub/data_lake/arnet/tables/citation_rwr/iter-"
    ...
    def main(args: Array[String]) = {
        ...
        var g = graph.partitionBy(
            PartitionStrategy.EdgePartition2D).mapVertices((id, attr) => (default_rank, 0, 0))
        ...
        g = g.mapVertices((id, attr) => {
            if (attr._2 == 0 || attr._3 == 0) {
                ((resProb * default_restart_vec), attr._2, attr._3)
        ...
        loop.breakable {
            while (true) {
                ...
                g = g.ops.joinVertices(msgs) {
                    (id, oldAttr, newAttr) => {
                        ((resProb * default_restart_vec) + (damFac * newAttr), oldAttr._2, oldAttr._3)
                    }
                }
