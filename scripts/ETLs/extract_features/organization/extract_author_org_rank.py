...
dst_dir                 = "gs://clpub/data_lake/arnet/tables/author_org_rank/merge-0"
...
author_org_rank_df = spark.sql(f"""
    ...
    from unique_author_df as phd
        ...
        ) org_r on org_r.org_name = phd.author_org
""")
author_org_rank_df.createOrReplaceTempView("author_org_rank")

author_group_org_rank = spark.sql(f"""
    select aor.author_id, cast(AVG(aor.org_rank) as float) as org_rank
    from author_org_rank as aor
    group by aor.author_id
""")

author_group_org_rank.write.mode("overwrite").save(dst_dir)
