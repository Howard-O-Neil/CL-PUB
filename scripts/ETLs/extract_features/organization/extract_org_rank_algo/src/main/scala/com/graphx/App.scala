...
object App {
    ...
    val dst_dir                 = "gs://clpub/data_lake/arnet/tables/org_rank_algo/iter-"
        ...
        loop.breakable {
            while (true) {
                ...
                val msgs = g.aggregateMessages[Float](
                    triplet => {
                        val ranking = (triplet.srcAttr._1.floatValue / triplet.srcAttr._3.floatValue) *
                            (triplet.attr.floatValue / triplet.srcAttr._5.floatValue)
                        triplet.sendToDst(ranking)
                    },
                    (a, b) => a + b
                )
                ...
                g = g.ops.joinVertices(msgs) {
                    (id, oldAttr, newAttr) => {
                        val restart_vect = 1f / (math.log(oldAttr._4.doubleValue)).floatValue
                        ((restart_vect * resProb) + (damFac * newAttr), oldAttr._2, oldAttr._3, oldAttr._4, oldAttr._5)
                    }
                }
