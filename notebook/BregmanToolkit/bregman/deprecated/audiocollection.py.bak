...
class AudioCollection:
    ...
    def __init__(self, path=None, root_path=DATA_ROOT):
        ...
        self.uid="%016X"%int(random.random()*100000000000000000) # unique instance ID
    ...
    def _gen_adb_hash(self):
        ...
        """
        m = hashlib.md5()
        k = list(self.feature_params.keys())
    ...
    def _insert_features_into_audioDB(self):
        ...
        fList, pList, kList = list(zip(*self.rTupleList))
    ...
    def extract_features(self, key=None, keyrepl=None, extract_only=False, wave_suffix=".wav"):
        ...
        self._fftextract_list(list(zip(aList,fList,pList,kList)))
        self.rTupleList = list(zip(fList,pList,kList)) # what will be inserted into audioDB
        if not extract_only:
...
#        lcoef = "%d"%self.feature_params['lcoef'] # not used yet        
    ...
    def ls_collections(cls, expand=False, limit=None):
        ...
        dlist = list(zip(dlist[:limit], tm[:limit]))
        if expand:
            R = cls()
            k = R.toc(dlist[0][0])
            for d,t in dlist: 
                print(d, t)
                print(list(k[0][1].keys()))
                for i, v in enumerate(R.toc(d)):
                    print("[%d]"%i, list(v[1].values()))
                print("")
        return dlist
