...
gcloud dataproc clusters create clpub-5478 \
    ...
    --project clpub-2 \
    --optional-components HIVE_WEBHCAT,JUPYTER,ZEPPELIN,ZOOKEEPER \
...
gcloud dataproc clusters create clpub-5478 \
    ...
    --zone asia-east1-a \
    --service-account clpub-453@clpub-2.iam.gserviceaccount.com \
    --single-node --master-machine-type e2-highmem-8 \
    --master-boot-disk-size 1000 --image-version 2.0-debian10 \
    --bucket clpub-cluster \
    --project clpub-2 \
    --optional-components HIVE_WEBHCAT,JUPYTER,ZEPPELIN,ZOOKEEPER \
...
gcloud dataproc clusters create recsys-a3aa --bucket clpub-cluster \
    ...
    --optional-components HIVE_WEBHCAT,ZEPPELIN,ZOOKEEPER \
    --scopes 'https://www.googleapis.com/auth/cloud-platform' \
    --master-min-cpu-platform "Intel Skylake" --project clpub-2
# >>>>>

# <<<<< Submit job from local <<<<<
...
# Single node / single coordinator

# Trino require Java 17, recommend Azul Zulu SDKs
...
# Single node setup
# Act as coordinator, also a worker
# Worker max memory = Whole cluster max memory
...
# Trino DO NOT use HiveQL, Trino make use of Hive metastore for accessing datasource
echo -e "\
connector.name=hive
...
sudo bash -c "JAVA_HOME=/opt/java-17 PATH=/opt/java-17/bin/:$PATH /opt/trino/bin/launcher run"
sudo bash -c "JAVA_HOME=/opt/java-17 PATH=/opt/java-17/bin/:$PATH /opt/trino/bin/launcher status"
...
./trino --server=localhost:8098 --catalog=hive --schema=default

# >>>>>

# <<<<< Mount cloud hard disk <<<<<

sudo lsblk # find your disk, most likely will be /dev/sdb

mkdir /home/howard/recsys
sudo mount /dev/sdb /home/howard/recsys
sudo chmod -R a+rwx  /home/howard/recsys
